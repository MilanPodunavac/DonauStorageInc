...
public interface PersonRepository extends JpaRepository<Person, Long>, JpaSpecificationExecutor<Person> {
    ...
    @Query(
        value = "select distinct person from Person person left join fetch person.contactInfo",
        countQuery = "select count(distinct person) from Person person"
    )
    Page<Person> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct person from Person person left join fetch person.contactInfo")
    List<Person> findAllWithToOneRelationships();

    @Query("select person from Person person left join fetch person.contactInfo where person.id =:id")
