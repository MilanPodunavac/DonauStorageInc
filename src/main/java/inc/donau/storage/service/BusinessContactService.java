...
public class BusinessContactService {
    ...
    /**
     *  Get all the businessContacts where BusinessPartner is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<BusinessContactDTO> findAllWhereBusinessPartnerIsNull() {
        log.debug("Request to get all businessContacts where BusinessPartner is null");
        return StreamSupport
            .stream(businessContactRepository.findAll().spliterator(), false)
            .filter(businessContact -> businessContact.getBusinessPartner() == null)
            .map(businessContactMapper::toDto)
            .collect(Collectors.toCollection(LinkedList::new));
    }
